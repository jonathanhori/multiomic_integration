...
# metric_list = []
metric_table_concat = pd.DataFrame()
        ...
        for params in train_grid:
            ...
            for rep in range(reps):
                ...
                model_out_filename = os.path.join(
                    model_out_path, 
                    model_out_filename_base.format(n, p, snr_x, snr_y, sparsity, k_delta, rep) + "_" \
                        + ".".join(f'{key}{val}' for key, val in train_config.items())
                        )
                check_metric_out_filename = os.path.join(
                    metric_out_path, 
                    metric_out_filename_base.format(n, p, snr_x, snr_y, sparsity, k_delta, rep) + "_" \
                        + ".".join(f'{key}{val}' for key, val in train_config.items())
                        )
                if os.path.isfile(check_metric_out_filename + ".csv"):
                ...
                try:
                    ...
                    eval_metric_table.to_csv(metric_out_filename + "_" \
                        + ".".join(f'{key}{val}' for key, val in train_config.items()) \
                        + ".csv")
                    
                    metric_table_concat = pd.concat([metric_table_concat, eval_metric_table])
                    # metric_list.append(eval_metric_table)
                except Exception as e:
                    print(e)
    
    metric_table_concat.to_csv(metric_out_filename + "_concat.csv")